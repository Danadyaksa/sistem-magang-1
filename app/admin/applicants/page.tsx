...
export default function ApplicantsPage() {
  ...
  const getPositionName = (id: number | null) => {
    if (!id) return "Bidang Terkait";
    return positions.find((p) => p.id === id)?.title || "Bidang Terkait";
  };

  const sendWhatsApp = (pelamar: Pendaftaran) => {
    const phone = formatPhoneNumber(pelamar.nomorHp);
    const posisi = getPositionName(pelamar.positionId);
    const text = `Halo, ${pelamar.namaLengkap}! Selamat, pengajuan magang kamu di Disdikpora telah DITERIMA pada posisi ${posisi}. Silakan cek email untuk detail selanjutnya. Terima kasih.`;
  ...
  const sendEmail = async (pelamar: Pendaftaran) => {
    const posisi = getPositionName(pelamar.positionId);
    toast.loading("Mengirim email...", { id: "sending-email" });
    ...
    try {
      const res = await fetch("/api/notify", {
        ...
        body: JSON.stringify({
          to: pelamar.email,
          subject: "PENGUMUMAN HASIL SELEKSI MAGANG DISDIKPORA",
          html: `
              <h3>Halo, ${pelamar.namaLengkap}</h3>
              <p>Selamat! Berdasarkan review berkas, kamu dinyatakan <b>DITERIMA</b> magang.</p>
              <p><b>Posisi:</b> ${posisi}</p>
              <p>Silakan datang ke kantor pada tanggal mulai magang yang telah ditentukan.</p>
              <br/>
              <p>Salam,<br/>Admin Disdikpora</p>
            `,
        }),
      });
      if (res.ok) {
  ...
  return (
    <div className="h-screen w-full bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300 overflow-hidden">
      {/* SIDEBAR */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-slate-900 text-white shadow-xl flex flex-col h-full transition-all duration-300 ease-in-out ${
                        ...
                        <TableCell className="text-right pr-4 py-3">
                          ...
                          >
                            ...
                            <DialogContent className="max-w-2xl bg-white dark:bg-slate-950 p-0 overflow-hidden border-slate-200 dark:border-slate-800">
                              ...
                              <DialogFooter className="p-4 bg-slate-100/50 dark:bg-slate-900/50 border-t border-slate-200 flex flex-col sm:flex-row gap-2 sm:gap-0 justify-between">
                                {item.status === "PENDING" ? (
                                  ...
                                  <div className="flex w-full items-center justify-between">
                                    ...
                                    {/* TOMBOL PERMANEN BUAT YG DITERIMA */}
                                    {item.status === "ACCEPTED" && (
                                      <div className="flex gap-2">
                                        <Button
                                          size="sm"
