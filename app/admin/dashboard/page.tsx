      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="bg-white border-b h-16 flex items-center px-4 md:px-8 justify-between sticky top-0 z-40">
          <div className="flex items-center gap-4">
            <button className="md:hidden p-2 hover:bg-slate-100 rounded" onClick={() => setSidebarOpen(true)}>
              <Menu className="h-6 w-6 text-slate-600" />
            </button>
            <h2 className="text-lg font-semibold text-slate-800">Overview Kuota</h2>
          </div>
          <div className="flex items-center gap-4">
            <div className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold border border-blue-200">A</div>
          </div>
        </header>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {isLoading ? "..." : positions.reduce((acc, curr) => acc + curr.quota, 0)}
                </div>
                <p className="text-xs text-slate-500">Kursi magang keseluruhan</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                {!isLoading && positions.length > 0 && (
                  <div className="text-xs font-bold text-slate-900 bg-slate-100 px-2 py-1 rounded">
                    {Math.round((positions.reduce((acc, curr) => acc + curr.filled, 0) / positions.reduce((acc, curr) => acc + curr.quota, 0)) * 100)}%
                  </div>
                )}
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {isLoading ? "..." : positions.reduce((acc, curr) => acc + curr.filled, 0)}
                </div>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  filteredPositions.map((pos) => {
                    ...
                    return (
                      <TableRow key={pos.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={`${status === 'Dibuka' ? 'bg-green-50 text-green-700 border-green-200' : ''} ${status === 'Terbatas' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : ''} ${status === 'Penuh' ? 'bg-red-50 text-red-700 border-red-200' : ''}`}>
                            {status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-blue-700" onClick={() => openEditModal(pos)}><Pencil className="h-4 w-4" /></Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-red-700" onClick={() => handleDelete(pos.id)}><Trash2 className="h-4 w-4" /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    );
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="h-24 text-center text-slate-500">Belum ada data.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Sesuaikan nama bidang dan jumlah kuota magang di sini.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="name">Nama Bidang</Label>
              <Input id="name" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} placeholder="Contoh: Sub Bagian Umum" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="filled">Terisi</Label>
                <Input id="filled" type="number" min={0} value={formData.filled} onChange={(e) => setFormData({...formData, filled: parseInt(e.target.value) || 0})} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="quota">Total Kuota</Label>
                <Input id="quota" type="number" min={1} value={formData.quota} onChange={(e) => setFormData({...formData, quota: parseInt(e.target.value) || 0})} />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button type="submit" className="bg-blue-700 hover:bg-blue-800" onClick={handleSave} disabled={isSubmitting}>
