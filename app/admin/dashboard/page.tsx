...
export default function AdminDashboard() {
  ...
  // Ambil semua state & fungsi dari Hook useAdminDashboard
  const {
    isLoading,
    isSubmitting,
    admin,
  ...
  return (
    <div className="h-screen w-full bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
          <div className="w-full">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Kelola kuota posisi, unit UPT, kalender hari libur, dan minimal magang.
            </p>
          </div>
          ...
          <div className="w-full">
            <Tabs defaultValue="positions" onValueChange={setActiveTab} className="w-full">
              ...
              <TabsList className="grid w-full grid-cols-4 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-1 mb-6 rounded-lg h-12 shadow-sm transition-colors overflow-x-auto">
                <TabsTrigger
                  value="positions"
                  className="data-[state=active]:bg-blue-50 dark:data-[state=active]:bg-blue-900/20 data-[state=active]:text-blue-700 dark:data-[state=active]:text-blue-400 font-medium rounded-md h-full transition-all dark:text-slate-400 text-sm whitespace-nowrap px-3"
                >
                    ...
                    <TableHeader className="bg-slate-50/80 dark:bg-slate-950">
                      <TableRow className="border-slate-200 dark:border-slate-800">
                        ...
                        <TableHead className="cursor-pointer" onClick={() => requestSort("title")}>
                          Nama Bidang <ArrowUpDown className="h-3 w-3 inline ml-1" />
                        </TableHead>
                        <TableHead className="text-center cursor-pointer" onClick={() => requestSort("filled")}>
                          Terisi/Kuota <ArrowUpDown className="h-3 w-3 inline ml-1" />
                        </TableHead>
                          ...
                          <TableRow key={pos.id} className="border-slate-100 dark:border-slate-800 hover:bg-slate-50/50">
                            ...
                            <TableCell className="text-right pr-4">
                              <Button
                                ...
                                onClick={() => {
                                  setPositionForm({ title: pos.title, quota: pos.quota });
                                  setEditingId(pos.id);
                                  setIsPositionDialogOpen(true);
                                }}
                              >
              ...
              <TabsContent value="upt" className="animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex justify-end mb-4">
                  <Button
                    onClick={() => {
                      setUptForm({ name: "", address: "" });
                      setEditingId(null);
                      setIsUptDialogOpen(true);
                    }}
                    className="bg-blue-700 hover:bg-blue-800 text-white shadow-lg shadow-blue-700/20 transition-all hover:scale-105"
                  >
                          ...
                          <TableRow key={u.id} className="border-slate-100 dark:border-slate-800 hover:bg-slate-50/50">
                            ...
                            <TableCell className="text-right pr-4">
                              <Button
                                ...
                                className="h-8 w-8 text-slate-400 hover:text-blue-600"
                                onClick={() => {
                                  setUptForm({ name: u.name, address: u.address || "" });
                                  setEditingId(u.id);
                                  setIsUptDialogOpen(true);
                                }}
                              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <Card className="border-slate-200 dark:border-slate-800 shadow-sm dark:bg-slate-900 h-fit lg:col-span-1">
                    ...
                    <CardContent>
                      <form onSubmit={handleAddHolidays} className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Popover>
                            <PopoverTrigger asChild>
                              <Button
                                variant={"outline"}
                                className={cn("w-full justify-start text-left font-normal dark:bg-slate-950 dark:border-slate-700")}
                              >
                        ...
                        <Button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 text-white mt-4">
                  ...
                  <Card className="border-slate-200 dark:border-slate-800 shadow-sm dark:bg-slate-900 lg:col-span-2">
                    <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b dark:border-slate-800">
                      <div>
                        <CardTitle className="text-lg">Daftar Tanggal Merah</CardTitle>
                      </div>
                      <div className="flex gap-2">
                        <Select value={filterYear} onValueChange={setFilterYear}>
                          <SelectTrigger className="w-[90px] h-9 dark:bg-slate-950">
                            <SelectValue />
                          </SelectTrigger>
                        ...
                        <Select value={filterMonth} onValueChange={setFilterMonth}>
                          <SelectTrigger className="w-[110px] h-9 dark:bg-slate-950">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent className="dark:bg-slate-950">
                            <SelectItem value="all">Semua</SelectItem>
                            ...
                            <TableRow className="border-slate-200 dark:border-slate-800">
                              <TableHead className="w-[60px] text-center bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-sm">
                                <input
                                  ...
                                  checked={selectedHolidayIds.length === holidays.length && holidays.length > 0}
                                  onChange={toggleSelectAllHolidays}
                                />
                              </TableHead>
                              <TableHead className="bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-sm">Tanggal</TableHead>
                            ...
                            {isLoading ? (
                              <TableRow>
                                <TableCell colSpan={3} className="h-24 text-center">
                                  <Loader2 className="animate-spin h-4 w-4 inline" /> Loading...
                                </TableCell>
                              </TableRow>
                            ) : holidays.map((item) => (
                              <TableRow
        ...
        <DialogContent className="dark:bg-slate-950 dark:border-slate-800">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                type="number"
                value={positionForm.quota}
                onChange={(e) => setPositionForm({ ...positionForm, quota: parseInt(e.target.value) || 0 })}
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={isUptDialogOpen} onOpenChange={setIsUptDialogOpen}>
        <DialogContent className="dark:bg-slate-950 dark:border-slate-800">
          <DialogHeader>
            <DialogTitle>{editingId ? "Edit UPT" : "Tambah UPT"}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input
                value={uptForm.name}
                onChange={(e) => setUptForm({ ...uptForm, name: e.target.value })}
            ...
            <div className="grid gap-2">
              <Label>Alamat</Label>
              <Input
                value={uptForm.address}
                onChange={(e) => setUptForm({ ...uptForm, address: e.target.value })}
                placeholder="Jl. Raya..."
              />
            </div>
          </div>
      ...
      <Dialog open={isDeleteMassOpen} onOpenChange={setIsDeleteMassOpen}>
        <DialogContent className="sm:max-w-[400px] dark:bg-slate-950 border-slate-800">
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2 mt-4">
            <Button variant="outline" className="w-full sm:w-1/2" onClick={() => setIsDeleteMassOpen(false)}>
              Batal
            </Button>
      ...
      <Dialog open={deleteConfirm.isOpen} onOpenChange={(open) => !open && setDeleteConfirm(prev => ({ ...prev, isOpen: false }))}>
        <DialogContent className="sm:max-w-[425px] p-6 border-slate-200 dark:border-slate-800 shadow-2xl bg-white dark:bg-slate-950">
          <div className="flex flex-col items-center text-center gap-2 pt-2">
            ...
            <DialogTitle className="text-xl font-semibold dark:text-slate-100">Hapus Data?</DialogTitle>
